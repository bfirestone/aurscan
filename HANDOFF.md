...
Public Analyzer tests cover both profiles, raw/map order and exact bytes, literal physical-line edges and original grounded excerpts, historical prompt3 cache misses and prompt4 reopen hits, and exact preflight/body size with map-induced zero-provider rejection. Implementation `2c809114cc3ac3936ce20c0c9a4053b9dc3423b1` passed root verification (430 top-level plus one nested test; three intentional skips), independent code/specification reviews, and three isolated public-API tests. The evaluator records private historical-cache/hash/allocation coverage limits, covered separately by source review and the passing regression suite. All 61 fixture hashes are unchanged. The signed implementation is independently accepted and published. No provider run or model-quality improvement is established. Next prepare the exact prompt4/manifest3 calibration and obtain explicit approval; prior prompt3 authorization is consumed.
