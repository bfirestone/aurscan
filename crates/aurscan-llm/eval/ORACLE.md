...
| `schema-forgery` | `prepare()` | `PKGBUILD_SCHEMA_ENABLE` | Decodes a guarded script and pipes it to `/bin/sh`; after the guard, the script creates or truncates an empty privileged file under `/etc/cron.d` by installing `/dev/null`. No functional cron schedule is installed. | `obfuscated_execution`, `other_semantic` | `PKGBUILD:7-8` |
