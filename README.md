...
The deterministic heuristic core is the supported product and remains the default. Experimental LLM analysis is off by default: only an explicit `aurscan deep-scan` or `aurscan ack --llm` can contact a model. LLM findings are **Advisory-only** and can never create a Block; they also cannot lower or remove deterministic findings. A missing LLM finding is not clearance.

The strict LLM configuration is read from `~/.config/aurscan/config.toml`. These examples are wire-compatible configuration examples, not qualification claims:
...
Export the selected secret into the environment of the process that launches `aurscan`; put only the environment-variable **name**, never its value, in TOML. An OpenAI-compatible base URL must expose the `/chat/completions` route expected by `aurscan` (these bases become `/v1/chat/completions`); model IDs and supported request fields vary by service. `standard` sends `max_tokens`, `temperature=0`, and `n=1`. `openai_reasoning_none` sends `max_completion_tokens`, `reasoning_effort="none"`, `temperature=0`, and `n=1`. Profiles are explicit, never inferred from endpoint/model names, and never automatically retried or substituted.

Remote use requires deliberate consent (`allow_remote = true`), HTTPS, and accepting the provider's terms and costs. Public recipe bytes plus tracked local modifications can be transmitted to the configured hosted endpoint. Review model compatibility and request pricing before running. The current prompt v4 candidate has not yet undergone live calibration or qualification. The Sol example demonstrates wire compatibility only; no tested model currently satisfies the frozen v1 qualification bar, and no accepted `crates/aurscan-llm/eval/reference-reports/v1.json` exists. Upstream semantic scanning and a shared distribution/cache service are not partially active in v1. See [`docs/experimental-llm.md`](docs/experimental-llm.md) for bundle boundaries, caps, cache behavior, acknowledgements, and JSON details.
...
For these commands, exit `0` means completed with no findings, `1` means Advisory, `2` means a deterministic Block, and `3` means an unavailable/incomplete analysis or configuration/operational error. Exit `3` can accompany rendered deterministic Blocks. Plain `aurscan ack` remains deterministic-only and never contacts an LLM.
